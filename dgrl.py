...
def read_from_dgrl(dgrl):
    if not os.path.exists(dgrl):
        print("DGRL does not exist, check file path.")
        return
    ...
    with open(dgrl, 'rb') as file:
        # 读取表头尺寸
        header_size = np.fromfile(file, dtype='uint8', count=4)
        header_size = sum([j << (i * 8) for i, j in enumerate(header_size)])
        # print(header_size)

        # 读取表头剩下内容，提取 code_length
        header = np.fromfile(file, dtype='uint8', count=header_size - 4)
        code_length = sum([j << (i * 8) for i, j in enumerate(header[-4:-2])])
        # print(code_length)

        # 读取图像尺寸信息，提取图像中行数量
        image_record = np.fromfile(file, dtype='uint8', count=12)
        ...
        for k in range(line_num):
            ...
            # 读取该行的字符数量
            char_num = np.fromfile(file, dtype='uint8', count=4)
            char_num = sum([j << (i * 8) for i, j in enumerate(char_num)])
            print('字符数量:', char_num)

            # 读取该行的标注信息
            """
            label = np.fromfile(file, dtype='uint8', count=code_length * char_num)
            for i in range(code_length * char_num):
                label[i] << (8 * (i % code_length))
            for i in range(char_num):
                label[i] = sum(label[i * code_length:(i + 1) * code_length])
            for i in label:
                label = struct.pack('I', i).decode('gbk', 'ignore')[0]
...
"""
            ...
            # 读取该行的位置和尺寸
            pos_size = np.fromfile(file, dtype='uint8', count=16)
            ...
            # 读取该行的图片
            bitmap = np.fromfile(file, dtype='uint8', count=h * w)
...
if __name__ == '__main__':
    ...
    paths = ['.\\data\\dgrl\\HWDB1.0trn\\001.mpf']  # dgrl path
    for path in paths:
        read_from_dgrl(path)
